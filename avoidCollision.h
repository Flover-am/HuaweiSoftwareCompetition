...
//                    vector_polar Vab_result = polar_vec_sub(Vb_result, Va_result);
                ...
                alpha = (alpha < -PAI) ? alpha + 2 * PAI : alpha;
                float omiga = (fabs(alpha) <= PAI) ? alpha : PAI;
                if (!data::have_printed[r_a_index]){
                    printf("forward %d %f\n", r_a_index, Va_result.first);
                    printf("rotate %d %f\n", r_a_index, omiga);
                }
                logger.writeInfo("print" + to_string(r_a_index)+" _B");

                if (data::destList[r_a_index][0].first == (data::destList[r_b_index][0].first) ||
                    ra.stationID == rb.stationID || Xab_polar.first <= dis_min - dis_deta + 0.3) {
                    logger.writeInfo("机器人逆行，两者为" + to_string(r_a_index) + " ," + to_string(r_b_index));
                    if (!data::have_printed[r_b_index]){
                        printf("forward %d -2\n", r_b_index);
                        printf("rotate %d 1\n", r_b_index);
                    }
                    logger.writeInfo("print" + to_string(r_b_index)+" _C");
                } else {
                    float beta = rb.alpha - Vb_result.second;
                    beta = (beta > PAI) ? beta - 2 * PAI : beta;
                    beta = (beta < -PAI) ? beta + 2 * PAI : beta;
                    float omiga2 = (fabs(beta) <= PAI) ? beta : PAI;
...
//                    && r_a_index == 0 && r_b_index == 3) {
//                    logger.writeInfo("Frame = " + to_string(data::frame));
//                    logger.writeInfo("dA:===" + to_string(data::destList[r_a_index][0].first));
//                    logger.writeInfo("dB:===" + to_string(data::destList[r_b_index][0].first));
//                    logger.writeInfo("");
//                }

                data::have_printed[r_a_index] = true;
                data::have_printed[r_b_index] = true;
